...
class PASS_NICE:
    ...
    def __init__(self, cell_corp: Literal["SK", "KT", "LG", "SM", "KM", "LM"], proxy: Optional[str] = None):
        ...
        self._AUTH_TYPE: str = ""
        self._SERVICE_INFO: str = ""
        self._CERT_INFO_HASH: str = ""
        self._CAPTCHA_VERSION: str = ""
    ...
    async def retrieve_captcha(self) -> Result[bytes]:
        ...
        if self._AUTH_TYPE not in ["sms", "app_push"]:
            raise SessionNotInitializedError("현재 인증 방식은 캡챠를 사용하지 않습니다.")
    ...
    async def send_sms_verification(
        ...
        Examples:
        >>> await <Client>.send_sms_verification("홍길동", "0001013", "1", "01012345678", "123456")
    ...
    async def send_push_verification(
        ...
        phone_number, captcha_answer = self._verify_phone_and_captcha(phone_number, captcha_answer)
    ...
    async def create_qr_verification(self) -> Result[bytes]:
        ...
        if self._AUTH_TYPE != "app_qr":
            raise SessionNotInitializedError("QR 방식으로 세션을 초기화 해야합니다.") # QR 인증 세션 초기화 검증
        try:
    ...
    async def check_sms_verification(self, sms_code: str) -> Result[VerificationData]:
        ...
        # SMS 코드 검증
        if not (len(sms_code) == 6 and sms_code.isdigit()):
    ...
    async def check_push_verification(self) -> Result[VerificationData]:
        ...
        if str(response_json.get('code', '0001')) != "0000":
    ...
    def _verify_input(birthdate: str, phone_number: str, captcha_answer: str) -> tuple[str, str, str]:
        ...
        # 생년월일 검증
        if not (len(birthdate) == 6 and birthdate.isdigit()): # 생년월일이 숫자, 6자리가 아니라면
            if (len(birthdate) == 8 and birthdate.isdigit()): # 생년월일이 8자리라면 (NICE 형식에 맞추기 위해 6자리로 변환)
        ...
        # 캡챠 코드 검증 
        if not (len(captcha_answer) == 6 and captcha_answer.isdigit()): # 6자리 / 숫자 검증
            raise ValidationError("올바르지 않은 캡챠 코드를 입력하셨습니다.")

        return (birthdate, phone_number, captcha_answer)
    
    @staticmethod
    def _verify_phone_and_captcha(phone_number: str, captcha_answer: str) -> tuple[str, str]:
        """전화번호와 캡챠만 검증합니다. (PASS 앱 인증 등 생년월일 필요 없는 경우)"""
        phone_number = phone_number.replace("-", "")
        if not (len(phone_number) == 11 and phone_number.isdigit()):
            raise ValidationError("올바르지 않은 휴대전화번호를 입력하셨습니다.")
        
        if not (len(captcha_answer) == 6 and captcha_answer.isdigit()):
            raise ValidationError("올바르지 않은 캡챠코드를 입력하셨습니다.")
        
        return (phone_number, captcha_answer)
